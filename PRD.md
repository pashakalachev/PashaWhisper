...
Draft v3 · September 6, 2026 · Product requirements; implementation status tracked in docs/BUILD_STATUS.md
...
2. Grant microphone access. Grant Accessibility access for text-field positioning and, when implemented, automatic insertion; transcription and manual copying remain usable without it.
...
**Initial implementation slice:** Build a runnable native early version before calling the first working version complete. Include the brand, menu-bar toggle shortcut, recording overlay, local Whisper with speech detection, model downloads, API settings, explicit verified copying, configurable toggle shortcuts, caret-aware cat/waveform overlay, and no history. Automatic insertion, configurable hold shortcuts, streaming chunks, and compatibility testing remain required follow-up work. Manual Copy is a deliberate boundary for this slice, not fulfillment of automatic insertion requirements. Bundle a small starter model for immediate offline use; larger models remain optional downloads.
